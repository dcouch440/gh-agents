...
A well-defined, validated message protocol for inter-agent communication with structured message types that can be serialized, validated, and logged.

**Checkpoint**: Messages serialize/deserialize correctly, validation catches malformed messages, and message flow is traceable.
...
**Key files**:
- `src/agents/channels.rs` - Basic channel types (3.3)
