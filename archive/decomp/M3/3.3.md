...
> Inter-agent communication via channels with role-aware context.

## Goal

A working channel-based communication system where agents receive commands and send responses through typed tokio channels. Task assignments include role information that determines what context files to load.

**Checkpoint**: Can send a command to an agent via channel, agent receives it with role-specified context, and response comes back to dispatcher.
...
- **Task assignments include role information** that determines required reading

**Key files**:
- `src/agents/agent.rs` - Agent struct from Ticket 3.1
- `src/agents/roles.rs` - Role definitions from Ticket 3.4
...
- Requires Ticket 3.4 complete (Role system for role-aware context)
...
use super::roles::{RoleId, CommunicationStyle, OutputFormat};
...
- **Role Integration**: TaskAssignment includes `role_id` and TaskContext includes `role_context` for role-aware execution
- The RoleManager (from 3.4) loads required_reading files before task dispatch
- CommunicationStyle and OutputFormat from the role affect how the agent responds
...
- [ ] TaskContext includes required_reading and role_context
- [ ] RoleContext includes system_prompt, style, output_format
