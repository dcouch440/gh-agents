...
// import {AgentActivityDemo} from "../chat";
// import {TreeCanvas} from "../tree-renderer";
// import {pipelineToTree} from "../tree-renderer/adapters/pipelineAdapter";
// import {tasksToTree} from "../tree-renderer/adapters/taskAdapter";
...
//       stage_name: "plan",
//       input_definitions: {},
//       output_description: "execution plan",
//       output_schema: null,
//     },
//     {
//       stage_number: 2,
...
//       input_definitions: {},
//       output_description: "code changes",
//       output_schema: null,
//     },
//     {
//       stage_number: 3,
...
//       stage_name: "review",
//       input_definitions: {},
//       output_description: "review result",
//       output_schema: null,
//     },
//     {
//       stage_number: 4,
...
//       approval_required: false,
//       fan_out: false,
//       stage_name: "lint+test",
//       input_definitions: {},
//       output_description: "test results",
//       output_schema: null,
//     },
//   ],
// };
...
//   id: string,
//   status: Agent["status"],
// ): Agent[] => agents.map((a) => (a.id === id ? {...a, status} : a));
...
//     makeTask("t2", "Add JWT refresh endpoint", "normal", "pending", null, [
//       "t1",
//     ]),
//     makeTask("t3", "Write auth integration tests", "normal", "pending", null, [
//       "t1",
//       "t2",
//     ]),
...
//       ? {
//           ...u,
//           total_input: u.total_input + inTok,
//           total_output: u.total_output + outTok,
//           call_count: u.call_count + 1,
//         }
//       : u,
//   );
...
// ): Task[] =>
//   tasks.map((t) =>
//     t.id === id ? {...t, status, assigned_agent: agent ?? t.assigned_agent} : t,
//   );
...
//         user_id: "u1",
//         status: "running",
//         initial_task: "deploy auth refactor",
//         stage_outputs: {},
//         current_stage: 1,
//         started_at: new Date().toISOString(),
//         completed_at: null,
...
//         feed: addFeed(
//           s.feed,
//           "Pipeline deploy-auth started",
//           "system_notice",
//           "sys",
//         ),
//         tasks: setTaskStatus(s.tasks, "t1", "in_progress", "Atlas"),
//       };
//     },
//   },
...
//       feed: addFeed(
//         s.feed,
//         "Atlas: analyzing auth module structure",
//         "agent_report",
//         "a1",
//       ),
//       usage: addUsage(s.usage, "orchestrator", 4200, 1800),
//       stages: [makeExec(1, "plan", "a1", "running", 1000, 4200, 1800, false)],
//       run: s.run
//         ? {...s.run, total_input_tokens: 4200, total_output_tokens: 1800}
//         : null,
//     }),
//   },
...
//     at: 2500,
//     apply: (s) => {
//       const agents = setAgentStatus(
//         setAgentStatus(s.agents, "a1", "idle"),
//         "a2",
//         "working",
//       );
//       return {
...
//         feed: addFeed(
//           s.feed,
//           "Atlas: plan complete — 3 files to modify",
//           "task_completed",
//           "a1",
//         ),
//         stages: [
//           makeExec(1, "plan", "a1", "completed", 2500, 4200, 1800, true),
//           makeExec(2, "implement", "a2", "running", 0, 0, 0, false),
//         ],
...
//       feed: addFeed(
//         s.feed,
//         "Forge: modifying src/auth/middleware.rs",
//         "agent_report",
//         "a2",
//       ),
...
//       stages: [
//         s.stages[0]!,
//         makeExec(2, "implement", "a2", "running", 1000, 6100, 3200, false),
//       ],
//       run: s.run
//         ? {...s.run, total_input_tokens: 10300, total_output_tokens: 5000}
//         : null,
//     }),
//   },
...
//     apply: (s) => {
//       const agents = setAgentStatus(
//         setAgentStatus(s.agents, "a2", "idle"),
//         "a3",
//         "working",
//       );
//       return {
...
//         feed: addFeed(
//           s.feed,
//           "Forge: implementation complete — 3 files changed",
//           "task_completed",
//           "a2",
//         ),
...
//         stages: [
//           s.stages[0]!,
//           makeExec(2, "implement", "a2", "completed", 2500, 8500, 4000, true),
//           makeExec(3, "review", "a3", "running", 0, 0, 0, false),
//         ],
//         run: s.run
//           ? {
//               ...s.run,
//               current_stage: 3,
//               total_input_tokens: 12700,
//               total_output_tokens: 5800,
//             }
//           : null,
//         tasks: setTaskStatus(
//           setTaskStatus(s.tasks, "t1", "review", "Shield"),
//           "t2",
//           "in_progress",
//           "Forge",
//         ),
...
//         s.stages[0]!,
//         s.stages[1]!,
//         makeExec(3, "review", "a3", "running", 1500, 5200, 2100, false),
//       ],
//     }),
//   },
...
//         feed: addFeed(
//           s.feed,
//           "Shield: review passed — awaiting approval",
//           "milestone",
//           "a3",
//         ),
//         stages: [
//           s.stages[0]!,
//           s.stages[1]!,
//           makeExec(3, "review", "a3", "completed", 3000, 5200, 2100, true),
//         ],
//         run: s.run
//           ? {
//               ...s.run,
//               status: "waiting_for_approval",
//               total_input_tokens: 17900,
//               total_output_tokens: 7900,
//             }
//           : null,
//       };
//     },
...
//     apply: (s) => {
//       const agents = setAgentStatus(
//         setAgentStatus(s.agents, "a3", "idle"),
//         "a4",
//         "working",
//       );
//       return {
...
//         feed: addFeed(
//           s.feed,
//           "Approval granted — continuing pipeline",
//           "system_notice",
//           "sys",
//         ),
//         stages: [
//           ...s.stages.slice(0, 3),
//           makeExec(4, "lint+test", "a4", "running", 0, 0, 0, false),
//         ],
...
//       feed: addFeed(
//         s.feed,
//         "Lint: running clippy + cargo test",
//         "agent_report",
//         "a4",
//       ),
...
//       stages: [
//         ...s.stages.slice(0, 3),
//         makeExec(4, "lint+test", "a4", "running", 1000, 1800, 600, false),
//       ],
//       run: s.run
//         ? {...s.run, total_input_tokens: 19700, total_output_tokens: 8500}
//         : null,
//     }),
//   },
...
//         stats: computeStats(agents),
//         feed: addFeed(
//           addFeed(s.feed, "Lint: all checks passed", "task_completed", "a4"),
//           "Pipeline deploy-auth completed",
//           "milestone",
//           "sys",
//         ),
...
//         stages: [
//           ...s.stages.slice(0, 3),
//           makeExec(4, "lint+test", "a4", "completed", 2500, 2600, 1000, true),
//         ],
//         run: s.run
//           ? {
//               ...s.run,
...
//               total_input_tokens: 20500,
//               total_output_tokens: 8900,
//             }
//           : null,
//         tasks: setTaskStatus(
//           setTaskStatus(s.tasks, "t2", "completed", "Forge"),
//           "t3",
//           "in_progress",
//           "Shield",
//         ),
...
//       ...s,
//       wsConnected: false,
//       feed: addFeed(s.feed, "WebSocket disconnected", "error", "sys"),
//     }),
//   },
//   {
//     at: 13800,
...
//       ...s,
//       wsConnected: true,
//       feed: addFeed(s.feed, "WebSocket reconnected", "system_notice", "sys"),
//     }),
//   },
// ];
...
// function TreeRendererShowcase({
//   pipeline,
//   run,
//   stages,
//   tasks,
//   agents,
// }: TreeRendererShowcaseProps) {
