...
impl RouterService {
    pub fn new(tool_router_repo: Arc<dyn ToolRouterRepo>, context_repo: Arc<dyn ContextStoreRepo>, request_repo: Arc<dyn RouterRequestRepo>, llm: Arc<dyn LLMProvider>) -> Self {
        Self {
            tool_router_repo,
            context_repo,
            request_repo,
    ...
    pub async fn route_request(&self, router_id: Uuid, session_id: Uuid, agent_execution_id: Option<Uuid>, intent: &str, conversation_context: &str, priority: &str) -> Result<RouteResult> {
        ...
        let tool_rows = self.tool_router_repo.get_router_tools(router_id).await.context("failed to load router tool assignments")?;
        ...
        let response = self.llm.send_message(request).await.map_err(|e| anyhow::anyhow!("router LLM call failed: {}", e))?;
        ...
        let status_str = if decision.tool.is_some() { "routed" } else { "no_action" };
        let chain_val = decision.chain.as_ref().and_then(|c| serde_json::to_value(c).ok());
        self.request_repo
        ...
        match &decision.tool {
            Some(_tool_name) => {
                if decision.is_async {
                    let passdown = decision.passdown.unwrap_or_else(|| "Working on it...".to_string());
                    Ok(RouteResult::Async { passdown, request_id: req_row.id })
                } else {
            ...
            None => Ok(RouteResult::NoAction {
                reason: decision.reason.unwrap_or_else(|| "No tool matched the intent.".to_string()),
            }),
        }
    }
    ...
    pub async fn store_result_as_context(&self, session_id: Uuid, request_id: Uuid, tool_name: &str, result: &str) -> Result<()> {
        ...
        self.request_repo
            .update_router_request(request_id, None, None, false, None, None, "completed", Some(truncate(result, 1000)))
            .await
    ...
    pub async fn load_session_context(&self, session_id: Uuid, limit: u32) -> Result<String> {
        ...
        let expired = self.context_repo.expire_stale_context(session_id).await.unwrap_or(0);
        if expired > 0 {
            tracing::debug!(session_id = %session_id, expired, "expired stale context entries");
        }

        let entries = self.context_repo.get_active_context(session_id, limit).await.context("failed to load session context")?;
...
fn parse_router_decision(content: &str) -> Result<RouterDecision> {
    ...
    serde_json::from_str::<RouterDecision>(&json_str).context("failed to parse router decision JSON")
}
