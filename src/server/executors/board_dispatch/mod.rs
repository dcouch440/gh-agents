//! Background board dispatch task runner.
//!
//! Spawns the ExecutionEngine with a BoardDispatchStrategy to dispatch
//! configuration instructions to per-node builders after Phase 0. Runs
//! with DispatchStreamSink and broadcasts WebSocket events for lifecycle tracking.
...
use crate::server::hub::strategies::BoardDispatchStrategy;
...
/// Run a background board dispatch task to completion.
///
/// Called from `tokio::spawn` in the board submit handler when the changeset
/// has meaningful changes worth dispatching. Builds a BoardDispatchStrategy,
/// runs the engine loop, and updates the TaskRegistry with the outcome.
///
/// `session_id` is the persistent board dispatcher session — the instruction
/// and outcome are persisted as messages so subsequent dispatches see history.
pub async fn run_board_dispatch_task(
    state: AppState,
    execution_id: Uuid,
    workflow_id: Uuid,
    user_id: UserId,
    ...
    let cancel_token = match state.task_registry().get_task(execution_id) {
        ...
        None => {
            tracing::error!(
                ...
                "Board dispatch task not found in registry"
            );
            return;
        }
    };

    // Persist the instruction as a user message in the dispatcher session.
    if let Err(e) = state
        .repos()
        .sessions
        ...
        .await
    {
        tracing::warn!(
            ...
            error = %e,
            "Failed to persist board dispatch instruction"
        );
    }

    let strategy = match BoardDispatchStrategy::new(
        state.clone(),
        workflow_id,
        user_id,
        instruction.clone(),
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::error!(
                execution_id = %execution_id,
                error = %e,
                "Failed to build BoardDispatchStrategy"
            );
    ...
    let ae_id = match state
        ...
        .create_agent_execution(CreateAgentExecutionInput {
            execution_type: ExecutionType::BoardDispatch,
            agent_id: None,
        ...
        .await
    {
        ...
        Err(e) => {
            tracing::warn!(
                execution_id = %execution_id,
                error = %e,
                "Failed to create agent execution record for board dispatch"
            );
            None
        }
    };
...
/// Persist the dispatch outcome as an assistant message in the dispatcher session.
async fn persist_outcome(state: &AppState, session_id: Uuid, user_id: UserId, content: &str) {
        ...
        .await
    {
        tracing::warn!(
            session_id = %session_id,
            error = %e,
            "Failed to persist board dispatch outcome"
        );
    }
}
...
async fn persist_trace(
    ...
    if let Some(task) = state.task_registry().get_task(execution_id) {
        if let Ok(trace_json) = serde_json::to_value(&task.trace) {
            if let Err(e) = state
                ...
                .await
            {
                tracing::warn!(ae_id = %ae_id, error = %e, "Failed to persist board dispatch trace");
            }
        }
    }
    ...
    if let Err(e) = state
        ...
        .await
    {
        tracing::warn!(ae_id = %ae_id, error = %e, "Failed to update board dispatch agent execution status");
    }
}
