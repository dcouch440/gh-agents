...
mod tests {
    ...
    #[test]
    fn build_current_state_with_files() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("topology.json"), "{}").unwrap();
        std::fs::write(dir.path().join("config.json"), "{}").unwrap();

        let state = build_current_state(dir.path());
        assert!(state.contains("topology status=\"present\""));
        assert!(state.contains("config status=\"configured\""));
    }
