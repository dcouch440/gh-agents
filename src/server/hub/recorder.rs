...
impl<'a> ExecutionRecorder<'a> {
    pub fn new(repo: &'a dyn ServerRepo, agent_execution_repo: Option<&'a dyn AgentExecutionRepo>, token_ledger_repo: Option<&'a dyn TokenLedgerRepo>) -> Self {
        Self {
            repo,
    ...
    pub async fn record_chat_message(&self, user_id: UserId, session_id: Option<Uuid>, message_id: Uuid, role: &str, content: &str) -> Result<(), HubError> {
        match session_id {
            Some(sid) => {
                self.repo
    ...
    pub async fn update_agent_execution(&self, execution_id: Uuid, status: &str, output: Option<String>, structured_output: Option<serde_json::Value>) -> Result<(), HubError> {
        let repo = self.agent_execution_repo.ok_or_else(|| anyhow::anyhow!("agent_execution_repo not configured"))?;
    ...
    pub async fn record_execution_message(&self, agent_execution_id: Uuid, role: &str, content: &str, tool_call_id: Option<String>, input_tokens: i64, output_tokens: i64) -> Result<(), HubError> {
        let repo = self.agent_execution_repo.ok_or_else(|| anyhow::anyhow!("agent_execution_repo not configured"))?;
        repo.create_execution_message(agent_execution_id, role, content, tool_call_id, input_tokens, output_tokens)
            .await
    ...
    pub async fn record_tokens(&self, user_id: Uuid, agent_execution_id: Option<Uuid>, model_id: &str, input_tokens: i64, output_tokens: i64, cost_usd: f32) -> Result<(), HubError> {
        let repo = self.token_ledger_repo.ok_or_else(|| anyhow::anyhow!("token_ledger_repo not configured"))?;
...
mod tests {
    ...
    async fn record_chat_message_global() {
        ...
        recorder.record_chat_message(UserId::new(), None, Uuid::new_v4(), "user", "hello").await.unwrap();
    }
    ...
    async fn record_chat_message_session() {
        ...
        let recorder = ExecutionRecorder::new(&mock, None, None);
        recorder.record_chat_message(UserId::new(), Some(session_id), Uuid::new_v4(), "assistant", "hi there").await.unwrap();
    }
    ...
    async fn record_tokens_without_repo_fails() {
        ...
        let recorder = ExecutionRecorder::new(&mock, None, None);
        let result = recorder.record_tokens(Uuid::new_v4(), Some(Uuid::new_v4()), "claude-3", 100, 50, 0.01).await;
    ...
    async fn record_tokens_with_repo() {
        ...
        tl_mock.expect_insert_ledger_entry().returning(|uid, aeid, _model, inp, out, cost| {
        ...
        recorder.record_tokens(Uuid::new_v4(), Some(Uuid::new_v4()), "claude-3", 100, 50, 0.01).await.unwrap();
    }
