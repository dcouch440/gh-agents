...
impl ValidationResult {
    ...
    pub fn is_ok(&self) -> bool {
        matches!(
            self.status,
            ValidationStatus::Ok | ValidationStatus::ApproachingLimit { .. }
        )
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ApproachingLimit {
                ...
                write!(
                    f,
                    "Warning: {} tokens (threshold: {}, limit: {})",
                    tokens, threshold, limit
                )
            }
            ...
            Self::ExceedsMaxContext { tokens, limit } => {
                write!(
                    f,
                    "Exceeds max context: {} tokens > {} limit",
                    tokens, limit
                )
            }
        }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::HistoryTruncated {
                ...
                write!(
                    f,
                    "Removed {} history entries ({} tokens)",
                    entries_removed, tokens_saved
                )
            }
            ...
            Self::ExamplesReduced {
                ...
                write!(
                    f,
                    "Reduced examples: kept {}, removed {} ({} tokens)",
                    kept, removed, tokens_saved
                )
            }
        }
    }
        ...
        match &result.status {
            ...
            ValidationStatus::ApproachingLimit {
                ...
                Some(ContextWarning {
                    ...
                    suggestion:
                        "Consider reducing history or reference files if more context needed."
                            .to_string(),
                })
            }
            ...
            ValidationStatus::ExceedsSafeLimit { tokens, limit } => Some(ContextWarning {
                ...
                suggestion: "Will attempt automatic truncation. Consider removing reference files."
                    .to_string(),
            }),
            ...
            ValidationStatus::ExceedsMaxContext { tokens, limit } => Some(ContextWarning {
                ...
                suggestion: "Must reduce context. Remove files or summarize large content."
                    .to_string(),
            }),
        }
    }
...
mod tests {
    ...
    fn test_response_length_estimates() {
        ...
        assert_eq!(
            counter.estimate_response_tokens(ResponseLength::Medium),
            1024
        );
        assert_eq!(counter.estimate_response_tokens(ResponseLength::Long), 4096);
        assert_eq!(
            counter.estimate_response_tokens(ResponseLength::VeryLong),
            8192
        );
        assert_eq!(
            counter.estimate_response_tokens(ResponseLength::Custom(500)),
            500
        );
    }
    ...
    fn test_validation_status_display() {
        ...
        let exceeds_safe = ValidationStatus::ExceedsSafeLimit {
            tokens: 185_000,
            limit: 180_000,
        };
        assert!(format!("{}", exceeds_safe).contains("Exceeds safe limit"));

        let exceeds_max = ValidationStatus::ExceedsMaxContext {
            tokens: 220_000,
            limit: 200_000,
        };
    ...
    fn test_warning_exceeds_safe() {
        let limits = ModelLimits::for_model("claude-sonnet");
        let result = ValidationResult {
            status: ValidationStatus::ExceedsSafeLimit {
                tokens: 185_000,
                limit: 180_000,
            },
    ...
    fn test_warning_exceeds_max() {
        let limits = ModelLimits::for_model("claude-sonnet");
        let result = ValidationResult {
            status: ValidationStatus::ExceedsMaxContext {
                tokens: 210_000,
                limit: 200_000,
            },
    ...
    fn test_warning_for_category_exceeded() {
        let warning =
            ContextPressureWarning::for_category(ContextCategory::FilesToModify, 120, 100);
        assert!(warning.is_some());
        assert_eq!(warning.unwrap().severity, WarningSeverity::Medium);
    }
