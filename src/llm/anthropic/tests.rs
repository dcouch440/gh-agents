...
mod tests {
    ...
    fn test_adapter_with_base(api_key: &str, base_url: &str) -> AnthropicAdapter {
        AnthropicAdapter {
            config: AnthropicConfig {
                api_key: api_key.to_string(),
    ...
    fn endpoint_url_correct() {
        let adapter = test_adapter("test-key", "claude-3");
        assert_eq!(
    ...
    fn build_request_uses_config_model_when_empty() {
        let adapter = test_adapter("test-key", "claude-3-opus");
        let llm_request = LLMRequest::new("", vec![Message::user("Hello")]);
        let body = adapter.build_request_body(&llm_request, false);
        assert_eq!(body["model"], "claude-3-opus");
    }

    #[test]
    fn build_request_uses_provided_model() {
        let adapter = test_adapter("test-key", "default-model");
    ...
    fn parse_sse_ignores_non_data_lines() {
        ...
        let empty_line = "";
        let result = adapter.parse_sse_line(empty_line);
        assert!(result.is_none());
    }
    ...
    fn handle_error_401_auth_error() {
        ...
        let error = adapter.handle_error(401, body, None);
        match error {
    ...
    fn handle_error_429_rate_limited() {
        ...
        let error = adapter.handle_error(429, body, None);
        match error {
    ...
    fn handle_error_429_with_retry_after() {
        ...
        let body =
            r#"{"type":"error","error":{"type":"rate_limit_error","message":"Too many requests"}}"#;
        let error = adapter.handle_error(429, body, Some(30000));
        match error {
    ...
    fn handle_error_500_api_error() {
        ...
        let error = adapter.handle_error(500, body, None);
        match error {
    ...
    fn handle_error_unparseable_body() {
        let adapter = test_adapter("key", "model");
        let error = adapter.handle_error(502, "not json at all", None);
        match error {
            LLMError::ApiError { status, message } => {
    ...
    fn handle_error_empty_body() {
        let adapter = test_adapter("key", "model");
        let error = adapter.handle_error(500, "", None);
        match error {
            LLMError::ApiError { status, message } => {
                assert_eq!(status, 500);
                assert_eq!(message, "");
    ...
    fn client_model_accessor() {
        ...
        let client = AnthropicClient::with_config(config).unwrap();
    ...
    fn build_request_with_system_prompt() {
        ...
        // stream=false is skipped during serialization (skip_serializing_if)
        assert!(body.get("stream").is_none() || body["stream"] == false);
    }
