...
<tool_descriptions>
  <tool name="create_pipeline">
    Creates a linear chain of workforce nodes with edges wired in order.
    Each step is created as a protocol node with its own assistant session.
    Use when the user describes a sequential flow — "first X, then Y,
    then Z." Returns the created topology.

    If any named step already exists, the tool returns a disregard response
    indicating no changes were made for that step.

    Parameters:
      steps: Array of { name: string } — the steps in pipeline order.
  </tool>

  <tool name="create_parallel">
    Creates a fan-out/fan-in pattern — one source node feeding multiple
    parallel nodes that converge into a single target node. All nodes are
    workforce protocol nodes. Use when the user wants independent work
    streams that merge into a synthesis step.

    If source or target is null, only that half of the pattern is created.

    Parameters:
      source: string | null — name of the source node (null if fan-in only)
      parallel: Array of { name: string } — the parallel worker nodes
      target: string | null — name of the convergence node (null if fan-out only)
  </tool>

  <tool name="insert_step">
    Splices a new workforce node into an existing edge. The original edge
    is removed and two new edges are created: from → new step → to.
    Use when the user wants to add a step between two existing steps.

    Parameters:
      between: { from: string, to: string } — the existing edge to splice into
      step: { name: string } — the new step to insert
  </tool>

  <tool name="remove_step">
    Removes a step from the workflow. If reconnect is true, upstream and
    downstream neighbors are wired together to maintain the data flow.
    If reconnect is false, the edges are simply removed.

    Parameters:
      name: string — step to remove
      reconnect: boolean — whether to bridge the gap (default: true)
  </tool>

  <tool name="update_step">
    Changes properties on an existing step — protocol, name, or other
    metadata. Use when the user wants to convert a step to a different
    protocol type or rename it.

    Parameters:
      name: string — current step name
      protocol: string (optional) — new protocol ("workforce", "meeting")
      new_name: string (optional) — rename the step
  </tool>

  <tool name="wire_edge">
    Adds a single directed edge between two steps. Use for custom topology
    adjustments that don't fit the pattern tools. Does not create nodes.

    If the edge already exists, returns a disregard response.

    Parameters:
      from: string — source step name
      to: string — target step name
  </tool>

  <tool name="dispatch_to_steps">
    Sends changesets to one or more step assistant sessions in a single
    call. Each step's assistant will auto-respond with a status and
    optional question. Use after topology changes to notify affected
    steps, or to relay user answers.

    If a step has already received an identical changeset, returns a
    disregard response for that step.

    Parameters:
      steps: Array of string — step names to dispatch to
      changeset_type: "initial_instruction" | "update" | "upstream_change"
                      | "feedback" | "peer_message"
      context: string — the changeset message body (plain English)
  </tool>

  <tool name="validate_dag">
    Checks the current workflow topology for issues: cycles, disconnected
    steps, missing port connections, unresolved questions. Returns a
    status per step and an overall pass/fail.
...
<tool_descriptions>
  <tool name="set_task">
    Sets the step's top-level task description. This is the mission
    statement that the Agent Designer reads when generating agent prompts.
    Keep it focused on WHAT this step should accomplish and WHY.

    If the task is already set to this value, returns a disregard response.

    Parameters:
      task: string — the task description
  </tool>

  <tool name="set_prompt">
    Sets or updates the step's prompt template. This is the raw prompt
    that gets sent to the execution engine. Use template variables like
    {{input}} for upstream data injection.

    Parameters:
      prompt: string — the prompt template
  </tool>

  <tool name="set_output_schema">
    Defines the structure of this step's output. Downstream steps use
    this schema to understand what data they'll receive via ports.
    Changes to the output schema may create beliefs for downstream steps.

    Parameters:
      schema: object — JSON schema for the step's output
  </tool>

  <tool name="add_agent">
    Adds an agent to the workforce roster. The Agent Designer will
    generate a tailored system prompt and task prompt for this agent
    based on the step's notes and dependency graph.

    Parameters:
      name: string — agent display name
      task: string — what this agent should accomplish
      capabilities: Array of string — tools this agent needs
  </tool>

  <tool name="remove_agent">
    Removes an agent from the workforce roster by name. Dependencies
    pointing to or from this agent are also removed.

    Parameters:
      name: string — agent to remove
  </tool>

  <tool name="set_dependency">
    Creates a dependency edge between two agents in the roster. The
    dependent agent receives the output of the agent it depends on.
    Without explicit dependencies, agents receive all prior outputs.

    Parameters:
      from: string — agent whose output is needed
      to: string — agent that needs the output
  </tool>

  <tool name="set_capabilities">
    Sets the available capabilities for this step. These determine which
    tools agents in this step can access during execution.

    If the capabilities are already set to these values, returns a
    disregard response.

    Parameters:
      capabilities: Array of string — capability keys from the palette
  </tool>

  <tool name="update_notes">
    Updates the step's persistent notes. Notes survive across
    conversations and are the primary context source for the Agent
    Designer. Include: objectives, constraints, technical details,
    decisions, and document references.
