...
describe('CodeEditor', () => {
  ...
  it('applies custom className', () => {
    const { container } = render(<CodeEditor value="" onChange={() => {}} className="custom" />)
    const editor = container.firstChild as HTMLElement
    expect(editor).toHaveClass('custom')
  })

  it('applies custom height', () => {
    const { container, rerender } = render(<CodeEditor value="" onChange={() => {}} height="500px" />)
    const editor = container.firstChild as HTMLElement
  ...
  it('renders placeholder when provided and value is empty', () => {
    render(<CodeEditor value="" onChange={() => {}} placeholder="Type here..." />)
