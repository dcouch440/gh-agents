...
describe('SplitPane', () => {
  it('renders left and right content', () => {
    render(<SplitPane left={<p>Left content</p>} right={<p>Right content</p>} splitPercent={50} onMouseDown={vi.fn()} />)
    expect(screen.getByText('Left content')).toBeInTheDocument()
    expect(screen.getByText('Right content')).toBeInTheDocument()
  })

  it('applies className when provided', () => {
    const { container } = render(<SplitPane left={<p>L</p>} right={<p>R</p>} splitPercent={50} onMouseDown={vi.fn()} className="custom" />)
    expect(container.firstChild).toHaveClass('custom')
  })

  it('handle has onMouseDown', () => {
    const handleMouseDown = vi.fn()
    render(<SplitPane left={<p>L</p>} right={<p>R</p>} splitPercent={50} onMouseDown={handleMouseDown} />)
  ...
  it('left panel width style matches splitPercent', () => {
    const { container, rerender } = render(<SplitPane left={<p>L</p>} right={<p>R</p>} splitPercent={35} onMouseDown={vi.fn()} />)
    ...
    rerender(<SplitPane left={<p>L</p>} right={<p>R</p>} splitPercent={60} onMouseDown={vi.fn()} />)
