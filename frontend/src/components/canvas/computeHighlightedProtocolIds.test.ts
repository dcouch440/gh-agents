...
describe('computeHighlightedProtocolIds', () => {
  it('includes the id of a selected protocol node', () => {
    const nodes = [{ id: 'workforce-1', data: { isProtocol: true } }]
    const result = computeHighlightedProtocolIds(nodes)
    expect(result.has('workforce-1')).toBe(true)
  })

  it('does not include protocolStepId of a non-protocol member node', () => {
    const nodes = [{ id: 'doc-1', data: { protocolStepId: 'workforce-1' } }]
    const result = computeHighlightedProtocolIds(nodes)
    expect(result.has('workforce-1')).toBe(false)
  })

  it('selecting a member does not highlight the whole protocol group', () => {
    const nodes = [{ id: 'doc-1', data: { protocolStepId: 'workforce-1', isProtocol: false } }]
    const result = computeHighlightedProtocolIds(nodes)
  ...
  it('selecting both a protocol and a member only adds the protocol id', () => {
    const nodes = [
      { id: 'workforce-1', data: { isProtocol: true } },
      { id: 'doc-1', data: { protocolStepId: 'workforce-1' } },
      { id: 'doc-2', data: { protocolStepId: 'workforce-1' } },
    ]
    const result = computeHighlightedProtocolIds(nodes)
