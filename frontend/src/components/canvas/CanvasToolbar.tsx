...
function RunButton() {
  ...
  const runColor = runStatus === 'completed' ? ('success' as const) : runStatus === 'error' ? ('error' as const) : ('primary' as const)
  ...
  return (
    <Zoom in={dirty} timeout={300}>
      ...
      >
        ...
        <Tooltip title={justSaved ? 'Saved!' : 'Save changes (\u2318S)'} TransitionComponent={Fade} enterDelay={500} placement="top">
          <span data-testid="toolbar-save-button">
            <GradientButton
              ...
              color="primary"
              icon={<SaveOutlined sx={{ fontSize: 16, transition: 'transform 0.2s ease' }} />}
              minWidth={100}
            >
