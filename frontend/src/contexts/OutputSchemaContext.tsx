...
const initialState: OutputSchemaState = {
  schemas: [],
  loading: true,
  error: null,
};
    ...
    case ACTION.UPDATE_ONE:
      return {
        ...
        schemas: state.schemas.some((s) => s.id === action.schema.id)
          ? state.schemas.map((s) =>
              s.id === action.schema.id ? action.schema : s,
            )
...
type OutputSchemaContextValue = OutputSchemaState & {
  reload: () => Promise<void>;
  addSchema: (schema: OutputSchema) => void;
  removeSchema: (id: string) => void;
};

const OutputSchemaContext = createContext<OutputSchemaContextValue | null>(
  null,
);
...
function OutputSchemaProvider({children}: {children: ReactNode}) {
    ...
    try {
      ...
      if (mountedRef.current)
        dispatch({
          ...
          error:
            e instanceof Error ? e.message : "Failed to load output schemas",
        });
    }
  }, []);
  ...
  const addSchema = useCallback((schema: OutputSchema) => {
    dispatch({type: ACTION.UPDATE_ONE, schema});
  }, []);

  const removeSchema = useCallback((id: string) => {
    dispatch({type: ACTION.REMOVE_ONE, id});
  }, []);

  return (
    <OutputSchemaContext.Provider
      value={{...state, reload: load, addSchema, removeSchema}}
    >
      {children}
    </OutputSchemaContext.Provider>
  );
